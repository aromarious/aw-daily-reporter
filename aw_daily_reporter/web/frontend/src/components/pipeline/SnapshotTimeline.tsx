...
function getColor(item: TimelineItem): string {
	...
	// Customize seed for specific sources to improve color differentiation
	if (
		category &&
		...
		try {
			// Extract hostname for Web events to group same-site colors
...
export default function SnapshotTimeline({
	...
	const { chartData, timeRange, categories, attrMap } = useMemo(() => {
		...
		// For Raw Data Sources/Clipped: use category (source) as lanes
		// For Raw Data: use specific attribute lanes
		// For others: use fixed attribute lanes
		let categories: string[] = [];
		const categoryToIndex: Record<string, number> = {};
		let attrMap: { key: string; label: string }[] = [];

		if (isSourceStage) {
			// Extract unique sources from category field
			...
			// Order: Raw Data (Top) -> Metadata (Bottom)
			// Note: yAxis is inverted, so index 0 matches Top.
			attrMap = [
				{ key: "app", label: "App" },
				...
				{ key: "separator", label: "" }, // Visual separator
		...
		if (items.length === 0) {
			if (domainStart !== undefined && domainEnd !== undefined) {
				return {
					chartData: [],
					timeRange: [domainStart, domainEnd],
					categories,
					attrMap: [],
				};
			}
			return { chartData: [], timeRange: [0, 0], categories: [], attrMap: [] };
		}
		...
		if (isSourceStage) {
			// Source stages: one lane per source
			items.forEach((item) => {
			...
			// Other stages: fixed attribute lanes
			items.forEach((item) => {
				const start = new Date(item.timestamp).getTime();
				if (Number.isNaN(start)) return;
				const end = start + item.duration * 1000;
				...
				attrMap.forEach((attr, idx) => {
					if (attr.key === "separator") return; // Skip separator lane

					// Handle nested metadata.client
					const val =
						attr.key === "client"
					...
					if (
						...
						if (attr.key === "category") {
							color = getColor(item);
						} else if (attr.key === "app") {
							color = `hsl(${hashCode(String(val)) % 360}, 65%, 50%)`;
						} else if (attr.key === "project") {
							color = `hsl(${hashCode(String(val)) % 360}, 70%, 45%)`;
						} else if (attr.key === "client") {
							color = `hsl(${hashCode(String(val)) % 360}, 55%, 60%)`;
						} else {
							color = `hsl(${hashCode(String(val)) % 360}, 60%, 55%)`;
						}
		...
		return {
			chartData: data,
			timeRange: [finalMin, finalMax],
			categories,
			attrMap,
		};
	}, [snapshot, domainStart, domainEnd, isSourceStage]);
	...
	// Dynamic height based on categories if not fixed
	...
	const option: EChartsOption = {
		tooltip: {
			formatter: (params: unknown) => {
				...
				// Max width to prevent huge tooltips
				let html = `<div class="font-sans text-xs text-base-content/80 max-w-[320px]">`;

				// Header: App + Duration
				html += `<div class="flex justify-between items-start mb-2 pb-1 border-b border-base-content/10">`;
				...
				// Metadata Badges (Project, Category, Client, Status)
				html += `<div class="flex flex-wrap gap-1.5 mb-2.5">`;
				if (item.project) {
				...
				if (hasDetails) {
					html += `<div class="space-y-1.5 pt-2 border-t border-base-content/10 text-[11px]">`;
					if (item.language) {
					...
					if (item.file) {
						// Split path to show filename more prominently?
		...
		xAxis: {
			type: "time",
			// Use snapshot specific range. Ideally this should be synchronized across all charts.
			min: timeRange[0],
			max: timeRange[1],
			axisLabel: {
		...
		series: [
			{
				...
				markLine: !isSourceStage
					? {
							symbol: ["none", "none"],
							silent: true,
							z: -1, // Render behind data
							label: { show: false }, // Hide values like 5.5
							lineStyle: {
							...
							data: (() => {
								...
								const separatorIdx = attrMap.findIndex(
									(a) => a.key === "separator",
								);
								if (separatorIdx > 0) {
									// Draw line exactly at the separator lane
									return [{ yAxis: separatorIdx }];
								}
								return [];
							})(),
						}
