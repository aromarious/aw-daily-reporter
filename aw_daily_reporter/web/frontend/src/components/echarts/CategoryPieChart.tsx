...
export default function CategoryPieChart({
		...
		series: [
			{
				...
				data: sortedData.map((item) => ({
					...
					itemStyle: {
						color:
							customColors && customColors[item.name]
	...
	const handleEvents = {
		...
		legendselectchanged: (params: any, instance: any) => {
			// Prevent hiding items (keep them always selected)
			// params.name is the clicked legend name
			// params.selected is a map of name -> boolean
			const isSelected = params.selected[params.name];
			if (!isSelected) {
				// If it became unselected, select it back immediately
		...
		highlight: (params: any, instance: any) => {
			// レジェンドホバー時にツールチップを表示
			// 名前からdataIndexを検索
			if (params.name) {
				const dataIndex = sortedData.findIndex((d) => d.name === params.name);
				if (dataIndex >= 0) {
					// EChartsの内部処理と競合しないよう遅延実行
					setTimeout(() => {
						instance.dispatchAction({
							type: "showTip",
							seriesIndex: 0,
